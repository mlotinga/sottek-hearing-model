...
from sottek_hearing_model.shmSubs import (shmDimensional,
                                          shmResample,
                                          shmPreProc,
                                          shmOutMidEarFilter,
                                          shmAuditoryFiltBank,
...
def shmRoughnessECMA(p, sampleRateIn, axisN=0, soundField='freeFrontal',
    ...
    for chan in chanIter:
        ...
        # if waitBar:
        #     bandIter = tqdm(range(nBands),
        #                     desc="Modulation rates")
        # else:
        #     bandIter = range(nBands)
        ...
        #     futures = [executor.submit(shmSpectralWeighting, zBand, lBlock,
        #                                errorCorrection, resDFT1500, theta,
        #                                modWeightSpectraAvg[:, lBlock, zBand])
        #                for zBand in bandIter for lBlock in blockIter]

        # for future in as_completed(futures):
        #     (zBand, lBlock, modAmpBandBlock, modRateBandBlock) = future.result()
        #     modAmp[:, lBlock, zBand] = modAmpBandBlock
        #     modRate[:, lBlock, zBand] = modRateBandBlock
        ...
        # with ThreadPoolExecutor(max_workers=nThreads) as executor:
        #     futures = [executor.submit(shmFundamentalModRate, zBand, lBlock,
        #                                modRate[:, lBlock, zBand],
        #                                modAmpHiWeight[:, lBlock, zBand])
        #                for zBand in bandIter for lBlock in blockIter]

        # for future in as_completed(futures):
        #     (zBand, lBlock, modFundRateBandBlock, modMaxWeightBandBlock) = future.result()
        #     modFundRate[lBlock, zBand] = modFundRateBandBlock
        #     modMaxWeight[:, lBlock, zBand] = modMaxWeightBandBlock
